    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                #torch.save(model_ft, '/home/research/tongwu/glass/donemodel/adv_model_005.pkl')
            running_loss = 0.0
            running_corrects = 0
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                # print(inputs)
                # delta = pgd(model, inputs, labels, epsilon=0.02, alpha=0.004, num_iter=20, randomize=True)               
                # inputs = torch.cat((inputs, (inputs+delta).clamp(0,1)), dim=0)
                # print(inputs)
                #labels = torch.cat((labels,labels),dim=0)
                #print(labels)
                inputs = inputs + torch.randn_like(inputs, device='cuda') * 0.5
                optimizer.zero_grad()
                # forward
...
# torch.save(model_ft, '/home/research/tongwu/glass/donemodel/adv_modelfinal12.pkl')
